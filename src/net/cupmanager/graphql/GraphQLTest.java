...
import net.cupmanager.graphql.BoardRoomSchema.AddCommentInput;
import net.cupmanager.graphql.BoardRoomSchema.AddCommentPayload;
...
//									return toGraphQLObject(result);
	...
	static {
		...
		GraphQLObjectType CommentType = GraphQLObjectType.newObject()
				...
				.field(newFieldDefinition().name("author").type(UserType)
						.dataFetcher(authorFetcher)
						.build())
				.build();
		...
		GraphQLFieldDefinition addCommentField = newFieldDefinition()
			...
			.dataFetcher(new DataFetcher() {
					...
					public Object get(DataFetchingEnvironment environment) {
						...
						map.put("comment", comment); // TODO: serialize these too right?
...
//              .query(reflectQueryType(BoardRoomSchema.Root.class))
              .query(RootType)
	...
	public static void main(String[] args) {
		ExecutionResult result = new GraphQL(schema).execute(" query Test { boardroom { comments(id:1){author{name}, text} }}");
